...
int main(int argc, char *argv[]) {
    ...
    auto c = graph.addConstant<float>(FLOAT, {2}, {1.f, 1.f});
    auto w = graph.addConstant<float>(FLOAT, {2}, {1.f, 1.f});
    ...
    for (auto tileNum = 0u; tileNum < numTiles; tileNum++) {
        ...
        auto v = graph.addVertex(cs, "ContinousVertex", {
    ...
    std::cout << "result = " << std::setprecision(precision) << (hits/(iterations)) << std::endl;
