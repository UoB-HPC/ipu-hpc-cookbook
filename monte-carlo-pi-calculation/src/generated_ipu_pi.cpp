...
int main(int argc, char *argv[]) {
    ...
    auto ref = graph.addVariable(FLOAT, {chunk_size}, "x");\
    poputil::mapTensorLinearly(graph, ref);

    auto x = poprand::uniform(graph, NULL, 0, ref, FLOAT, 0.f, 1.f, map);
    auto y = poprand::uniform(graph, NULL, 0, ref, FLOAT, 0.f, 1.f, map);

    auto ipu_results = popops::map(graph, popops::expr::Cast(popops::expr::Lte(
                                                                popops::expr::Sqrt(
                                                                    popops::expr::Add(
                                         ...
                                         UNSIGNED_INT), {x, y}, map);

    auto copyToHostProgram = Copy(ipu_results, fromIpuStream);
    ...
    Program s = Repeat(iterations / chunk_size, Sequence{map, copyToHostProgram});
