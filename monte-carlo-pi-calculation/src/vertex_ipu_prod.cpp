...
int main(int argc, char *argv[]) {
    ...
    std::vector<float> wvec(dimensions);
    std::random_device dev;
    std::mt19937 mt(dev());
    std::uniform_real_distribution<float> dist(-.90f, .99f);
    std::uniform_real_distribution<float> dist_sqrt(0.9f, 0.99f);
    std::generate(cvec.begin(), cvec.end(), [&mt, &dist_sqrt]{return dist_sqrt(mt);});
    std::generate(wvec.begin(), wvec.end(), [&mt, &dist]{return dist(mt);});

    auto c = graph.addConstant<float>(FLOAT, {dimensions}, cvec);
    auto w = graph.addConstant<float>(FLOAT, {dimensions}, wvec);
    ...
    for (auto tileNum = 0u; tileNum < numTiles; tileNum++) {
        ...
        auto v = graph.addVertex(cs, "ProductPeakVertex", {
    ...
    auto engine = Engine(graph, Sequence({poplar::program::PrintTensor("print-c", c), poplar::program::PrintTensor("print-w", w),
                                          Execute(cs), Copy(out, fromIpuStream)}), ENGINE_OPTIONS);
